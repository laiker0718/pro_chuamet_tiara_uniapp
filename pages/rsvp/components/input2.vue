    ...
    <uni-transition mode-class="slide-right" :duration="500" :show="true" :styles="{'width': '100%', 'height': '100%'}">
        <view class="container"
            ...
            <view class="form"
                ...
                <view class="information">信息登记</view>

                <view class="name-content">
                    <view class="title">姓名</view>
                    <input class="input"
                        :style="{'border-bottom': isNameInvalid ? '1px solid #FF0000' : '1px solid #AAAAAA'}"
                        placeholder=" " :value="name" @input="onNameChange" />
                </view>
                <view class="phone-content">
                    <view class="title">手机</view>
                    <input class="input"
                        :style="{'border-bottom': isPhoneInvalid ? '1px solid #FF0000' : '1px solid #AAAAAA'}"
                        type="number" placeholder=" " :value="phone" @input="onPhoneChange" />
                </view>
                <view class="valid-code">
                    <view class="title">验证码</view>
                    <view class="valid-code-content">
                        <input class="valid-code-input"
                            :style="{'border-bottom': isVerifyCodeInvalid ? '1px solid #FF0000' : '1px solid #AAAAAA'}"
                            type="number" placeholder=" " :value="validCode" @input="onValidCodeChange" />
                ...
                <view class="privacy-content">
                    ...
                    <view class="privacy-text" :style="{'border-bottom': '1px solid #0B257A'}" @click="onBtnPrivacy">
                        个人信息处理规则</view>
                </view>
    ...
    export default {
        ...
        methods: {
            ...
            onBtnConfirm() {
                ...
                let dateStr = curDate.getFullYear() + '-' + monthStr + '-' + dayStr
                console.log('date :', dateStr)
                let timeStr = curDate.getHours() + ':00'
                console.log('time :', timeStr)
                getApp().globalData.userInfo.onsite = 0
                ...
                getApp().globalData.userInfo.stageInfo.rsvp.date = dateStr
                getApp().globalData.userInfo.stageInfo.rsvp.time = timeStr
                    ...
                    success: res => {
                        ...
                        if (res.data.status == 0) {
                            ...
                            if(getApp().globalData.from == 'Digital') {
                                that.$emit("onCompChange", 'Countdown')
                            } else {
                                // 活动已开始
                                if (getApp().globalData.userInfo.isActive) {
